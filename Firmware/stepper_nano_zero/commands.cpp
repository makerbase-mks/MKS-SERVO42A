...
calibrate - Calbirates the encoder, should be done with motor disconnected from machine
getcal - Prints the calibration table
testcal - tests the calibaration of the encoder
...
spid - with no arguments prints SIMPLE PID parameters, with arguments sets PID 'sPID Kp Ki Kd' Where Kp,Ki,Kd are floating point numbers
...
eepromerror - returns error in degreees from eeprom at power up realtive to current encoder
eepromloc - returns location in degreees eeprom on power up
eepromwrite - forces write of location to eeprom
setpos - sets the current angle in degrees
reboot - reboots the unit
eepromsetloc - sets the device angle based on EEPROM last reading, compenstates for error
